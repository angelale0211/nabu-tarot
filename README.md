...
**Course codes** – only for somebody who paid without an account. `#/admin` → *Mã học*, pick the course and the date it runs to, press *Tạo mã*, send the code. The app keeps no secret: the dashboard files a fingerprint of each code (a salted SHA-256) in `content/codes`, a document only Nabu and the worker can read. Redeeming needs an account and goes through the worker (`/redeem`, `worker/src/codes.ts`): it looks the code up, refuses one another account has already used, binds it to this account and writes the course onto the account itself, and the phone reads it back (`src/codes.js`). A code Nabu never issued opens nothing, and editing the course or the date inside a real code gets nowhere. Cancelling a code stops it being redeemed again — it does not take anything back from somebody who already used it; the Codes tab shows who used each code with a button to their thread in *Tin nhắn*, where their access can be withdrawn. Codes only work once the worker has `PLAY_SERVICE_ACCOUNT` and `FIREBASE_PROJECT_ID` set and `CONFIG.aiEndpoint` points at it.
