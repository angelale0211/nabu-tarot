...
The Play app is the live site wrapped as a Trusted Web Activity: it opens the same URL full screen, with no browser bar, and updates the moment the site updates. Build it without any local tooling at https://www.pwabuilder.com (enter the site URL, Package for stores, Android): it returns an `.aab`, a signing key and the SHA-256 fingerprint. Put that fingerprint into `well-known/assetlinks.template.json` and publish the file as `https://angelale0211.github.io/.well-known/assetlinks.json`, which means a repository named `angelale0211.github.io` with the file at `.well-known/assetlinks.json`, otherwise the app shows a browser bar. Inside the Play build `isTWA()` is true (android-app referrer), so course prices and the buy buttons are hidden and only unlock codes are taken, as Play's payment policy requires for digital content sold outside Play Billing. Privacy policy: `privacy.html` (built from `privacy.json`, also shown at `#/privacy`). Account deletion: Me → Xoá tài khoản (`BE.deleteAccount`), plus the email route in the policy.
