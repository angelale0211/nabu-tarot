...
| App activity | Other actions (Nabu AI questions and the last six turns of that conversation; no web search - the owner chose the free tier, where Google refuses it) | Yes | **Yes → Cloudflare (Workers AI writes answers for readers without a course) and Google (Gemini writes answers for readers with a course or Pro)**. Gemini runs on Google's unpaid API tier, under whose terms Google may use the question and answer to improve its products and people at Google may review them - so on the Play form this is data *shared*, not only processed by a service provider, and the privacy policy says so in three languages | App functionality | Optional | `src/ai.js`, `worker/src/index.ts` |
