...
STYLE = ('<style>body{margin:0;background:#EFE9FA;color:#3B2A5E;font-family:"Be Vietnam Pro","Segoe UI",Roboto,Arial,sans-serif;font-size:16px;line-height:1.6}'
         'main{max-width:680px;margin:0 auto;padding:28px 20px 48px}h1{font-family:Georgia,serif;font-weight:500;font-size:28px;margin:0 0 4px}'
         'h2{font-size:18px;margin:22px 0 6px}p{margin:0 0 10px}.d{color:#9C90B6;font-size:13px}.lead{color:#6B5C8A}hr{border:0;border-top:1px solid #DCD2EE;margin:36px 0}a{color:#3D2A6E}</style>')

def legal_page(src, out_name):
    P = json.load(io.open(os.path.join(HERE, src), encoding='utf-8'))
    def block(lg):
        return ('<h1>' + htmlmod.escape(P['title'][lg]) + '</h1><p class="d">' + htmlmod.escape(P['updated']) + '</p><p class="lead">' + htmlmod.escape(P['intro'][lg]) + '</p>'
                + ''.join('<h2>' + htmlmod.escape(s['h'][lg]) + '</h2><p>' + htmlmod.escape(s['p'][lg]) + '</p>' for s in P['sections']))
    page = ('<!DOCTYPE html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Nabu Tarot: '
            + htmlmod.escape(P['title']['vi']) + ' / ' + htmlmod.escape(P['title']['en']) + '</title>' + STYLE + '</head><body><main>'
            '<p><a href="./">\u2190 Nabu Tarot</a></p>' + block('vi') + '<hr>' + block('en') + '<hr>' + block('de') + '</main></body></html>')
    io.open(os.path.join(OUT, out_name), 'w', encoding='utf-8', newline='\n').write(page)
    print('%s: %d bytes' % (out_name, len(page.encode('utf-8'))))

legal_page('privacy.json', 'privacy.html')
legal_page('terms.json', 'terms.html')
