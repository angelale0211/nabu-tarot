  ...
  const show = (t) => {
    if (t === 'signs') {
      panel.innerHTML = '<div class="visual">' + zodiacRingSVG(me > -1 ? ZKEYS[me] : null) + '<div class="ellegend">' + ['fire', 'earth', 'air', 'water'].map((el) => '<span><i style="background:' + EL_COLOR[el] + '"></i>' + esc(ZELEM[el][lang]) + '</span>').join('') + '</div></div>'
        + '<div class="signlist">' + ZKEYS.map((k, i) => { const z = ZSIGN[k]; return '<button data-sign="' + k + '" class="' + (i === me ? 'me' : '') + '"><span class="g" style="color:' + EL_COLOR[z.el] + '">' + z.g + '</span><b>' + esc(z[lang]) + '</b><span>' + esc(lang === 'vi' ? z.dvi : (lang === 'de' ? (z.dde || z.den) : z.den)) + '</span></button>'; }).join('') + '</div>';
      ...
      panel.innerHTML = '<div class="visual awrapper"><div id="hwheel"></div><p class="faint">' + esc(lang === 'vi' ? 'Chạm vào một nhà trên bánh xe. Nhà 1 bắt đầu ở bên trái, chỗ cung Mọc.' : (lang === 'de' ? 'Tipp auf ein Haus im Rad. Haus 1 beginnt links am Aszendenten.' : 'Tap a house on the wheel. House 1 starts on the left, at the Ascendant.')) + '</p></div><div class="ins" id="hout"></div>';
      showHouse(1);
    } else if (t === 'aspects') {
...
function renderSign(key) {
  ...
  m.innerHTML = backLink('#/learn/astro', S.cats.astro) + '<div class="detail">'
    ...
    + '<div><div class="name">' + esc(z[lang]) + '</div><div class="en">' + esc(lang === 'vi' ? z.en : (lang === 'de' ? z.en : z.vi)) + '</div><div class="meta"><i>' + esc(lang === 'vi' ? z.dvi : (lang === 'de' ? (z.dde || z.den) : z.den)) + '</i></div></div></div>'
...
function spreadsListHTML(sys) {
  ...
  return list.map((s) => '<a class="acc" href="#/learn/spread/' + s.id + '" style="display:block;text-decoration:none;color:inherit"><button style="pointer-events:none"><span>' + esc(s.name) + ' <span class="faint">· ' + s.n + (lang === 'vi' ? ' lá' : (lang === 'de' ? ' Karten' : ' cards')) + '</span></span></button></a>').join('');
}
function renderSpread(id) {
  ...
  m.innerHTML = backLink(back, S.courseTabs.spreads) + '<div class="guide"><h1>' + esc(s.name) + '</h1><p class="faint">' + s.n + (lang === 'vi' ? ' lá' : (lang === 'de' ? ' Karten' : ' cards')) + '</p>'
...
function renderNumbers() {
  ...
  m.innerHTML = backLink('#/learn/fortune', S.cats.fortune) + '<h1 style="margin-bottom:12px">' + esc(S.numerology) + '</h1>'
    ...
    + '<div class="ins"><h3>' + esc(S.animal) + '</h3><p>' + (lang === 'vi' ? esc(animalOf(b.y).vi) + ' · ' + esc(canChi(b.y)) : (lang === 'de' ? 'Jahrestier: ' + esc(animalOf(b.y).de || animalOf(b.y).en) + ' (' + b.y + ')' : 'Year of the ' + esc(animalOf(b.y).en) + ' (' + b.y + ')')) + '</p><p class="faint">' + esc(S.animalNote) + '</p></div>'
