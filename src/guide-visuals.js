...
function guideVisualHTML(id) {
  ...
  if (v === 'woop') return '<div class="woop">' + [['W', lang === 'vi' ? 'Mong muốn' : 'Wish', '🌱'], ['O', lang === 'vi' ? 'Kết quả' : 'Outcome', '🌸'], ['O', lang === 'vi' ? 'Trở ngại' : 'Obstacle', '🪨'], ['P', lang === 'vi' ? 'Kế hoạch' : 'Plan', '🗺️']].map((x) => '<div class="wp"><span class="ic">' + x[2] + '</span><b>' + x[0] + '</b><span>' + esc(x[1]) + '</span></div>').join('') + '</div>';
  if (v === 'vision') return '<div class="vboard">' + [['💗', lang === 'vi' ? 'Tình cảm' : 'Love'], ['💼', lang === 'vi' ? 'Việc và tiền' : 'Work and money'], ['🌿', lang === 'vi' ? 'Sức khỏe' : 'Health'], ['🏡', lang === 'vi' ? 'Gia đình, bạn bè' : 'Family and friends'], ['📚', lang === 'vi' ? 'Học hỏi' : 'Learning'], ['🛌', lang === 'vi' ? 'Nghỉ ngơi' : 'Rest']].map((x) => '<div class="vb"><span class="ic">' + x[0] + '</span><b>' + esc(x[1]) + '</b><span class="ph">' + esc(lang === 'vi' ? 'dán ảnh của bạn' : 'your picture here') + '</span></div>').join('') + '</div>';
  if (v === 'moonritual') return '<div class="ritual"><div class="rt"><span class="ic">🌑</span><b>' + esc(lang === 'vi' ? 'Trăng non: gieo' : 'New Moon: plant') + '</b><span>' + esc(lang === 'vi' ? 'viết 3 ý định · đọc to · cất đi' : 'write 3 intentions · read aloud · put away') + '</span></div><span class="arr">→ 14 ' + esc(lang === 'vi' ? 'ngày' : 'days') + ' →</span><div class="rt"><span class="ic">🌕</span><b>' + esc(lang === 'vi' ? 'Trăng tròn: buông' : 'Full Moon: release') + '</b><span>' + esc(lang === 'vi' ? 'đọc lại · ghi bài học · xé tờ buông bỏ' : 'reread · note the lesson · tear up the release note') + '</span></div></div>';
  if (v === 'ifthen') return '<div class="ifthen">' + [[lang === 'vi' ? 'Nếu là 7 giờ sáng thứ Hai' : 'If it is 7 a.m. on Monday', lang === 'vi' ? 'thì mình mở hồ sơ xin việc' : 'then I open the application'], [lang === 'vi' ? 'Nếu muốn cầm điện thoại sau bữa tối' : 'If I reach for the phone after dinner', lang === 'vi' ? 'thì mình đi giày vào trước' : 'then I put my shoes on first'], [lang === 'vi' ? 'Nếu thấy sợ trước khi gọi' : 'If I feel scared before the call', lang === 'vi' ? 'thì mình hít 3 hơi rồi bấm gọi' : 'then I take 3 breaths and dial']].map((x) => '<div class="it"><span class="if">' + esc(x[0]) + '</span><span class="then">' + esc(x[1]) + '</span></div>').join('') + '</div>';
