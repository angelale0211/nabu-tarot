...
const PET_ART = {
  ...
  ninetails: () => ({
    ...
    only: (() => {
      ...
      let tails = '';
      const base = [86, 90];
      [[-64, 74], [-48, 82], [-32, 88], [-16, 90], [0, 88], [14, 84], [28, 78], [42, 70], [56, 60]].forEach((t, i) => {
      ...
      const head = '<path d="M50 30 Q36 30 26 38 Q16 45 12 50 Q10 53 13 55 L30 57 Q38 58 44 55 Q52 51 54 42 Q55 33 50 30 Z" fill="' + WHITE + '"/>'
        + '<path d="M26 44 Q18 47 13 51 Q11 53 14 54 L30 56 Q26 50 26 44 Z" fill="' + SHADE + '" opacity=".8"/>';
      /* Both ears sit on the skull and lean back, the far one behind and
         smaller, which is what makes the head read as turned away. */
      const ear = '<path d="M52 30 Q56 16 66 10 Q66 22 60 32 Z" fill="' + SHADE + '" stroke="#EBC9D6" stroke-width="1"/>'
        + '<path d="M43 32 Q45 12 57 4 Q59 20 53 33 Z" fill="' + WHITE + '" stroke="#EBC9D6" stroke-width="1.2"/>'
        + '<path d="M46 29 Q48 16 55 10 Q56 21 52 30 Z" fill="' + RED + '" opacity=".6"/>';
      const face = '<g class="eyes"><path d="M26 45 Q32 41 38 46 Q31 49 26 45 Z" fill="' + INK + '"/>'
        + '<circle cx="32" cy="45" r="1.3" fill="#fff"/></g>'
        + '<path d="M12 50 Q9 51 10 54 Q13 55 15 53 Z" fill="' + INK + '"/>'
        + '<path d="M16 55 q4 3 8 1" stroke="' + INK + '" stroke-width="1.1" fill="none" stroke-linecap="round"/>'
        /* the mask marks: a stroke over the eye and two on the cheek */
        + '<path d="M22 39 Q30 35 40 39" stroke="' + RED + '" stroke-width="2.6" fill="none" stroke-linecap="round"/>'
        + '<path d="M28 51 q6 2 10 0 M30 55 q5 1 8 0" stroke="' + RED + '" stroke-width="1.8" fill="none" stroke-linecap="round" opacity=".75"/>'
        + '<g stroke="#E8C4D2" stroke-width="1" stroke-linecap="round" opacity=".9"><path d="M18 46 h-8 M18 50 h-9"/></g>';
      ...
      return moon + tails + body + neckline + ear + head + face + charm;
    })()
  }),
  dragon: () => ({
