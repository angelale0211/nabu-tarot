...
function reportInfo() {
  return [CONFIG.brand + ' ' + (window.APP_VERSION || ''), location.hash, navigator.userAgent.replace(/\).*$/, ')'), (window.innerWidth + 'x' + window.innerHeight), lang, (typeof BILL !== 'undefined' && BILL.storefront ? 'storefront ' + BILL.storefront + (BILL.details && Object.keys(BILL.details).length && (BILL.details[Object.keys(BILL.details)[0]] || {}).price ? '/' + (BILL.details[Object.keys(BILL.details)[0]].price.currency || '?') : '') : ''), OOPS.length ? 'errors: ' + OOPS.join(' | ') : (window.__lastError ? 'error: ' + window.__lastError : '')].filter(Boolean).join(' · ');
}
