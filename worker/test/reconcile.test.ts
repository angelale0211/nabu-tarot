...
/* The catalogue's subscription skus (`PLAY_SUB_SKUS` in catalog.ts) are still
   the empty string - Input B1, not yet filled in by Play Console - so
   `itemBySku` can never match a "subs" row against the real catalogue today.
   That is exactly why the brief's own Step-1 test below is gated on the same
   condition as the one already-skipped subscription test in billing.test.ts.
   The tests after it need to actually run so they can prove reconciliation
   works and prove the self-healing claim - so they borrow one catalogue slot
   for the duration of a single test and put it back in `finally`, the way a
   filled-in sku will read once B1 lands for real. Mutating the item object is
   safe: PLAY_ITEMS is a module-scoped singleton private to this test file's
   own process (node's test runner isolates each *.test.ts file), so nothing
   here can leak into another test file's run. */
async function withSku<T>(key: string, sku: string, fn: () => Promise<T>): Promise<T> {
  const item = itemByKey(key)!;
  const had = item.sku;
  item.sku = sku;
  try { return await fn(); } finally { item.sku = had; }
}
...
test("a transient Google failure on one row never revokes it, is counted failed, and does not stop the next row", async () => {
  await withSku("plus", "test.plus.sub", () => withSku("pro", "test.pro.sub", async () => {
    const k = K;
    const docs: Record<string, Record<string, unknown>> = {
      "purchases/bad": fsDoc({ uid: "u1", sku: "test.plus.sub", kind: "subs", ids: ["plus"], state: "SUBSCRIPTION_STATE_ACTIVE", token: "BAD" }).fields as Record<string, unknown>,
      "purchases/ok": fsDoc({ uid: "u2", sku: "test.pro.sub", kind: "subs", ids: ["pro", "plus"], state: "SUBSCRIPTION_STATE_CANCELED", token: "OK" }).fields as Record<string, unknown>,
      "users/u1": fsDoc({ access: { plus: "2099-01-01" }, subs: { plus: { sku: "test.plus.sub", plan: "p", state: "SUBSCRIPTION_STATE_ACTIVE", until: "2099-01-01", autoRenew: true, tok: "h", opens: ["plus"], grant: true } } }).fields as Record<string, unknown>,
    ...
    const m = mockFetch(k, {
      ...
      "androidpublisher.googleapis.com": (url) => {
        ...
        return json({ subscriptionState: "SUBSCRIPTION_STATE_ACTIVE", acknowledgementState: "ACKNOWLEDGEMENT_STATE_ACKNOWLEDGED", lineItems: [{ productId: "test.pro.sub", expiryTime: okExpiry, autoRenewingPlan: { autoRenewEnabled: true }, offerDetails: { basePlanId: "pro-12m" } }] });
      },
      "firestore.googleapis.com": (url, init) => { const id = url.split("/documents/")[1].split("?")[0]; if (init.method === "PATCH") { docs[id] = Object.assign(docs[id] || {}, JSON.parse(String(init.body)).fields); return json({}); } return docs[id] ? json({ fields: docs[id] }) : json({}, 404); },
    });
    try {
...
test("SELF-HEALING: a subs row missing from users/{uid} (lost to a concurrent write) is restored by the next reconcile pass", async () => {
  await withSku("plus", "test.plus.sub", () => withSku("manifest", "test.manifest.sub", async () => {
    const k = K;
    ...
    const docs: Record<string, Record<string, unknown>> = {
      ...
      "purchases/lost": fsDoc({ uid: "u1", sku: "test.plus.sub", kind: "subs", ids: ["plus"], state: "SUBSCRIPTION_STATE_IN_GRACE_PERIOD", token: "S-LOST" }).fields as Record<string, unknown>,
      ...
      "users/u1": fsDoc({
        ...
        subs: { manifest: { sku: "test.manifest.sub", plan: "manifest-12m", state: "SUBSCRIPTION_STATE_ACTIVE", until: "2099-01-01", autoRenew: true, tok: "hman", opens: ["manifest"], grant: true } },
    ...
    const m = mockFetch(k, {
      ...
      "androidpublisher.googleapis.com": () => json({ subscriptionState: "SUBSCRIPTION_STATE_ACTIVE", acknowledgementState: "ACKNOWLEDGEMENT_STATE_ACKNOWLEDGED", lineItems: [{ productId: "test.plus.sub", expiryTime: expiry, autoRenewingPlan: { autoRenewEnabled: true }, offerDetails: { basePlanId: "plus-12m" } }] }),
      "firestore.googleapis.com": (url, init) => { const id = url.split("/documents/")[1].split("?")[0]; if (init.method === "PATCH") { docs[id] = Object.assign(docs[id] || {}, JSON.parse(String(init.body)).fields); return json({}); } return docs[id] ? json({ fields: docs[id] }) : json({}, 404); },
    });
    try {
      const out = await reconcileSubs(env(k));
